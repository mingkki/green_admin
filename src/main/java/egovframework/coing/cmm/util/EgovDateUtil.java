...
public class EgovDateUtil {
    ...
    public static String getCurrentDate(String dateType) {
        ...
        String strDate = Integer.toString(year) + ((month < 10)
                ...
                : Integer.toString(month)) + ((date < 10)
    ...
    /**
     * 파라미터로 받은 시작년부터 현재년까지를 리스트로 반환하는 함수
     * 현재 월일이 1201일보다 클경우 다음해도 리스트에 포함시킨다.
     * @param int startYear
     * @return List<String> yearList
     */
    public static List<Integer> getYearList(int startYear) {
        List<Integer> resultList = new ArrayList<>();
        String currYMD = EgovDateUtil.getToday();
        String currMD = currYMD.substring(4);
        int md = Integer.parseInt(currMD);
        if(md >= 1201) {
            String nextDate = EgovDateUtil.addYear(currYMD, 1);
            String nextYear = nextDate.substring(0, 4);
            resultList.add(Integer.parseInt(nextYear));
        }
        int currY = Integer.parseInt(currYMD.substring(0, 4));
        int cnt = currY - startYear;
        for(int i = currY; i >= startYear; i--) {
            resultList.add(i);
        }
        return resultList;
    }
