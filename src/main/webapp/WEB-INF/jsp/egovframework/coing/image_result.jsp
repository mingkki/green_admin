...
<script type="text/javascript">
	<c:choose>
		...
		<c:when test="${message == 'common.success.update'}">
			alert("<spring:message code='${message}' />");
			document.location.href = "?act=write&imgId=<c:out value='${searchImageInfoVO.imgId}' />&<c:out value='${param.returnQueryString}' escapeXml='false' />";
		</c:when>
		<c:when test="${message == 'common.success.delete'}">
			alert("<spring:message code='${message}' />");
			document.location.href = "?<c:out value='${searchImageInfoVO.queryString}' escapeXml='false' />";
		</c:when>
		<c:otherwise>
			alert("<spring:message code='${message}' />");
			history.back(-1);
