...
<form:form commandName="writeSurveyQuestion" action="?act=qwrite" class="form-horizontal" role="form">
	...
	<div class="block">
		...
		<div class="block-content">
			<c:if test="${result.svquType eq 'B'}">
				...
				<c:if test="${result.svquType ne 'C'}">
					<ul style="margin:0; padding:0;">
						...
						<li style="list-style:none;">
							...
							<c:if test="${result.svquType eq 'B'}"><input type="checkbox" name="questions[<c:out value='${result.svquId}' />]" value="<c:out value='${example.svexId}' />" /></c:if>
