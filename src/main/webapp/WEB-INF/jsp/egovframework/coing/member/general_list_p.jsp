				...
				<form action="${REQUEST_URI}" method="post" class="form-inline">
					<fieldset>
						...
						<div class="form-group">
							<select name="memGroups" id="memGroups" class="form-control">
							<option value="">+ 회원 그룹</option>
							<c:forEach var="result" items="${groupList}" varStatus="status">
							<option value="<c:out value='${result.grpId}'/>" <c:if test="${result.grpId eq searchMemberGeneralVO.memGroups}">selected="selected"</c:if>><c:out value="${result.grpName}"/></option>
							</c:forEach>
							</select> 
						</div>
						<div class="form-group">
				...
				<div class="block-content np">
					...
					<c:forEach var="result" items="${resultList}" varStatus="status">
						<tr>
							...
							<td class="text-center"><c:out value="${result.memGroupsNm}"/></td>
