...
public String generateTreemenu(List<SiteMenuVO> list, int parent) {
	...
	for (int i = 0; i < list.size(); i++) {
		int smeId = list.get(i).getSmeId();
		if (list.get(i).getSmeParntsId() == parent) {

			tree += String.format("<li class=\"mjs-nestedSortable-branch mjs-nestedSortable-expanded\" id=\"menuItem_%d\"><div class=\"menuDiv\"><span><span data-id=\"%d\" class=\"itemTitle\">", smeId, smeId);
			tree += String.format("%s</span></span></div>", list.get(i).getSmeName());
			tree += this.generateTreemenu(list, smeId);
			tree += "</li>";
		}
	}
