...
<script type="text/javascript">
  ...
  alert("<spring:message code='${message}' />");
  document.location.href = "?act=write&id=<c:out value='${searchVO.id}' />&<c:out value='${param.returnQueryString}' escapeXml='false' />";
  </c:when>
  <c:when test="${message == 'common.success.delete'}">
  alert("<spring:message code='${message}' />");
  document.location.href = "?<c:out value='${searchVO.id}' escapeXml='false' />";
  </c:when>
